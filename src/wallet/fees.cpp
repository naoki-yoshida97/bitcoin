...
CAmount GetMinimumFee(unsigned int nTxBytes, const CCoinControl& coin_control, const CTxMemPool& pool, const CBlockPolicyEstimator& estimator, FeeCalculation *feeCalc)
{
    ...
    */
    ...
    if (coin_control.m_feerate) { // 1.
        ...
        fee_needed = estimator.estimateSmartFee(target, feeCalc, conservative_estimate, coin_control.m_mempool_optimized).GetFee(nTxBytes);
        if (fee_needed == 0) {
...
CFeeRate GetDiscardRate(const CBlockPolicyEstimator& estimator)
{
    ...
    CFeeRate discard_rate = estimator.estimateSmartFee(highest_target, nullptr /* FeeCalculation */, false /* conservative */, false /* mempool optimized */);
