...
UniValue getnewblockhex(const JSONRPCRequest& request)
{
    ...
    if (request.params[1].isNull()) {
        ...
        reserve_script = coinbase_script->reserveScript;
    } else {
        util::insert(reserve_script, ParseHexV(request.params[1], "coinbase_script"));
    }

    std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlock(reserve_script));
    {
